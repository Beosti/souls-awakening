package com.yuanno.soulsawakening.particles.fire;
...
public class FireWaveParticleEffect extends ParticleEffect {
    @Override
    public void spawn(World world, double posX, double posY, double posZ, double motionX, double motionY, double motionZ)
    {
        ...
        while (phi < 1)
        {
            ...
            for (double theta = 0; theta <= 2 * Math.PI; theta += Math.PI / 64)
            {
                ...
                if(i % 3 == 0)
                    data = new GenericParticleData(ModParticleTypes.FIRE.get());
                else
                    data = new GenericParticleData(ModParticleTypes.FIRE.get());
                data.setLife(7);
                data.setSize(3F);

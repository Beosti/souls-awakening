...
public class ReishiTridentItem extends TridentItem {
    ...
    public static class ReishiSpearEvents {
        public static final UUID REISHI_TRIDENT_ATTACK_RANGE_ID = UUID.fromString("5f2a24fc-0613-11ef-9262-0242ac120002");
        ...
        public static void onReishiSpearRangeBonus(ItemAttributeModifierEvent event) {
            ...
            if (event.getSlotType() != EquipmentSlotType.MAINHAND)
                return;
            if (itemStack.getItem().asItem() instanceof ReishiTridentItem) {
                AttributeModifier mod = new AttributeModifier(REISHI_TRIDENT_ATTACK_RANGE_ID, "Trident Bonus", 0.4, AttributeModifier.Operation.ADDITION);
        ...
        public static void onAttackWithTrident(LivingDamageEvent event) {
            if (event.getEntityLiving() != null && !event.getEntityLiving().level.isClientSide) {
                if (event.getSource().getDirectEntity() != null) {
                    ...
                    if (!(livingEntity.getMainHandItem().getItem().asItem() instanceof ReishiTridentItem))
                        return;
        ...
        public static void onToolTipRender(ItemTooltipEvent event) {
            ...
            if (event.getItemStack().getItem() instanceof ReishiTridentItem && entityStats.getRace().equals(ModValues.QUINCY) && entityStats.getReiatsuPoints() > 0) {

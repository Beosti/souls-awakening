...
public class ShinigamiTeacherPrompt extends ChatPrompt {
    ...
    void dialogue2ShinigamiTeacher()
    {
        ...
        if (!chatPromptScreen.getEntityStats().getRace().equals(ModValues.SHINIGAMI))
            chatPromptScreen.setText("You shouldn't be here...");
        if (chatPromptScreen.getText().equals("Now you're a shinigami, I do have some missions for you. I got one where you have to rescue some pluses, are you interested?"))
            this.addAcceptanceDecline = true;
    }
    void dialogue3ShinigamiTeacher()
    {
        ...
        if (!chatPromptScreen.getEntityStats().getRace().equals(ModValues.SHINIGAMI))
            chatPromptScreen.setText("Where did you even come from?");
        if (chatPromptScreen.getText().equals("I got another mission for you, now you're officially part of the gotei 13 you can also be paid. It's about a specific hollow."))
            this.addAcceptanceDecline = true;
    }
    ...
    void shinigamiTeacherOnClose()
    {
        if (this.chatPromptScreen.getText().equals("Good job on your first kill! You have forged a better bond with your sword, making it have a spirit also making you a real shinigami. Feel free to walk around and learn new stuff")) {
            chatPromptScreen.getPlayer().sendMessage(new TranslationTextComponent("You can now increase your stats in the player overview screen and learn from other teachers."), Util.NIL_UUID);
            chatPromptScreen.getQuestData().getQuest(ModQuests.KILLHOLLOW).setInProgress(false);
        ...
        if (this.chatPromptScreen.getText().equals("Here's a blade called a 'zanpakuto', right now it's just an asauchi(without spirit) due to you not being aware of the spirit inside. You can press alt+right click with zanpakuto to go and back to the human world. Kill a hollow and I'll make you a shinigami.")) {
            chatPromptScreen.getQuestData().addInProgressQuest(ModQuests.KILLHOLLOW);
            PacketHandler.sendToServer(new CSyncQuestDataPacket(chatPromptScreen.getQuestData()));
            PacketHandler.sendToServer(new CSyncGiveQuestStartPacket(ModQuests.KILLHOLLOW));
            chatPromptScreen.getPlayer().sendMessage(new TranslationTextComponent("This entity is now a teleport point, you can teleport back to it in your teleports menu. You need to be in the same dimension to teleport."), Util.NIL_UUID);
        }
        if (this.chatPromptScreen.getText().equals("Great work as a shinigami to help those pluses. You definitely do deserve to be ranked in the gotei 13 now.")) {
            chatPromptScreen.getPlayer().sendMessage(new TranslationTextComponent("You have received the rank of non-officer official part of the gotei 13."), Util.NIL_UUID);
            chatPromptScreen.getQuestData().getQuest(ModQuests.RESCUE_PLUSES).setInProgress(false);
        ...
        if (this.chatPromptScreen.getText().equals("Great! You have to go to the overworld and rescue 5 pluses, they're lost spirits. Just right click them with your zanpakuto and they'll be saved!")) {
            chatPromptScreen.getQuestData().addInProgressQuest(ModQuests.RESCUE_PLUSES);
            PacketHandler.sendToServer(new CSyncQuestDataPacket(chatPromptScreen.getQuestData()));
        }
        if (chatPromptScreen.getText().equals("Amazing, you'll have to find the 'beast' hollow in the overworld. It's a hollow on 4 feet that walks around and is quite fast. It looks like a tiger. Kill it and come back for your reward!"))
        {
            chatPromptScreen.getQuestData().addInProgressQuest(ModQuests.KILL_SPECIFIC_HOLLOW);
            PacketHandler.sendToServer(new CSyncQuestDataPacket(chatPromptScreen.getQuestData()));
        }
        if (chatPromptScreen.getText().equals("There have some money, thanks for handling that hollow. I am sure it took you some time to hunt it down and track it's location.")) {
            chatPromptScreen.getQuestData().getQuest(ModQuests.KILL_SPECIFIC_HOLLOW).setInProgress(false);

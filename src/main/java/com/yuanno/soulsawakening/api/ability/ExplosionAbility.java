	...
	public void doExplosion()
	{
		...
		if (this.canDestroyBlocks && false && (this.explodedBlocksLimit <= 0 || this.explodedBlocks < this.explodedBlocksLimit))
		{
			...
			for (BlockPos blockpos : this.affectedBlockPositions)
			{
				...
				if (!blockstate.isAir() && fallingProtection)
				{
		...
		if (this.canStartFireAfterExplosion && false)
		{
			...
			while(positions.hasNext())
			{
				...
				if (this.world.getBlockState(blockpos1).isAir() && this.world.getBlockState(blockpos1.below()).isSolidRender(this.world, blockpos1.below()) && this.random.nextInt(5) == 0)
				{
